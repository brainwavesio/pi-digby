        ...
        if (event.type === "tool_execution_start") {
            ...
            if (debugThreading) {
                const label = pending?.args
                    ? ((pending.args as Record<string, unknown>).label as string)
                    : undefined;
            ...
            if (event.result) {
                log.info(
                    `[${channelId}] Compaction complete: ${event.result.tokensBefore} tokens compacted`,
                );
            ...
            const e = event as any;
            log.warn(
                `[${channelId}] Retrying (${e.attempt}/${e.maxAttempts}): ${e.errorMessage}`,
            );
