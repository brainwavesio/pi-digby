...
import { createTools, type ToolContexts } from "../tools/index.js";
...
import type { SlackEvent, SlackChannel, SlackUser } from "../slack/types.js";
...
export async function createChannelRunner(opts: {
    ...
    return {
        async run(
            ...
            const syncedCount = syncLogToContext(
                sessionManager,
                channelDir,
                event.ts,
                event.threadTs,
            ...
            toolContexts.attach.uploadFn = async (
                filePath: string,
                title?: string,
            ) => {
            ...
            try {
                ...
                const timeoutPromise = new Promise<void>((_, reject) => {
                    setTimeout(
                        () => reject(new Error(`Run timed out after ${getRunTimeout()}s`)),
                        timeoutMs,
                    );
                });
                ...
                if (stats.stopReason === "error" && stats.errorMessage) {
                    ctx.replaceMessage(
                        `_Sorry, something went wrong: ${truncate(stats.errorMessage, 500)}_`,
                    );
                    ...
                    const sessionFinalText =
                        lastAssistant?.content
                            ...
                            .map((c) => c.text)
                            .join("\n") || "";
                    const finalText = sessionFinalText.trim()
                        ? sessionFinalText
                        : stats.lastStreamedText;
        ...
        async shutdown(): Promise<void> {
            ...
            if (runner) {
                try {
                    ...
                    log.warn(
                        `[${channelId}] Extension shutdown error: ${err instanceof Error ? err.message : String(err)}`,
                    );
                }
            }
        },
